...
static unique_ptr<TableRef> TableBindReplace(ClientContext &context, TableFunctionBindInput &input) {
	...
	if (input.inputs.size() != 1) {
		throw ParserException("Expected a table or a list with tables as input");
	}
	string query;
	union_tables_query(input, query);
	auto subquery_ref =
	    parse_subquery(query, context.GetParserOptions(), "Expected a table or a list of tables as input");
	return subquery_ref;
}
